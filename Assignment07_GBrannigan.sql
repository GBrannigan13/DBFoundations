...
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_GBrannigan')
	 Begin 
	  Alter Database [Assignment07DB_GBrannigan] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_GBrannigan;
	 End
	Create Database Assignment07DB_GBrannigan;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_GBrannigan;
...
Select
  ...
  UnitPrice
  From vProducts;
Select ProductName, '$' + Cast(UnitPrice as varchar (max)) 
From vProducts;
go
Select ProductName, '$' + str(UnitPrice, 5, 2) as UnitPrice 
From vProducts
...
--Select
--  p.ProductName,
--  i.InventoryDate Format (getdate(), 'MMMM-yyyy') as date,
--  i.[Count];
--From vProducts as p
...
Select
  p.ProductName,
  [InvetoryDate] = DateName(MM, i.InventoryDate) + ' , ' + DateName(YY, i.InventoryDate),
